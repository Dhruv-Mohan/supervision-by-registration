...
def load_all_G16k(root_dir):
    print('G16k Root Dir : {}'.format(root_dir))
    with open('/home/dhruv/datasets/landmark-datasets/G16k/Bounding_Boxes/G16k.pickle', 'rb') as pick_in:
        data_dict = pickle.load(pick_in)
...
def generate_300w_list(root, save_dir, box_data, SUFFIX):
  ...
  train_length, common_length, challenge_length = 14784, 1219, 0
  subsets = ['G16k']
  dir_lists = [osp.join(root, subset) for subset in subsets]
  imagelist, num_image = load_list_from_folders(dir_lists, ext_filter=['jpg'], depth=3)
...
if __name__ == '__main__':
  ...
  SAVE_DIR = osp.join(this_dir, 'lists', 'G16k')
  print ('This dir : {}, HOME : [{}] : {}'.format(this_dir, HOME_STR, os.environ[HOME_STR]))
  path_300w = osp.join( os.environ[HOME_STR], 'datasets', 'landmark-datasets', 'G16k')
  USE_BOXES = ['GTB', 'DET']
  print(path_300w)

  box_datas = load_all_G16k(path_300w)
